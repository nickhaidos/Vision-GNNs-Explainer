...
class DeepGCN(torch.nn.Module):
    ...
    def forward(self, inputs):
        ...
        B, C, H, W = x.shape
        edge_indexes = []
        # print('x after steam and pos: ', x.shape)
        steam_feature = x
        block_features = []

        for i in range(self.n_blocks):
            x, edge_index = self.backbone[i](x)
            edge_indexes.append(edge_index)
            block_features.append(x)
            # print('x after block', i, x.shape)

        x = F.adaptive_avg_pool2d(x, 1)
        # print('x after avg_pool', x.shape)
        return self.prediction(x).squeeze(-1).squeeze(-1), edge_indexes, steam_feature, block_features
